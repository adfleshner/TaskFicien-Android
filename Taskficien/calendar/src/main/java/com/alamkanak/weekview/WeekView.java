...
public class WeekView extends View {
    ...
    private Paint mHeaderRightTextPaint;
    private float mHeaderRightTextHeight;
    ...
    private Paint mTodayHeaderRightTextPaint;
    ...
    private int mTodayHeaderTextColor = Color.parseColor("#2292e1");
...
//        mTimeTextHeight = 28;
        ...
        mHeaderRightTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mHeaderRightTextPaint.setColor(mHeaderColumnTextColor);
        mHeaderRightTextPaint.setTextAlign(Paint.Align.CENTER);
        mHeaderRightTextPaint.setTextSize(mTextSize-38);
        mHeaderRightTextPaint.getTextBounds("0", 0, "0".length(), rect);

        mHeaderRightTextHeight = rect.height();
        mHeaderTextPaint.setTypeface(Typeface.createFromAsset(mContext.getAssets(),"fonts/Exo-Medium.otf"));
        mHeaderRightTextPaint.setTypeface(Typeface.createFromAsset(mContext.getAssets(),"fonts/Exo-Medium.otf"));
        ...
        mTodayHeaderTextPaint.setTextAlign(Paint.Align.RIGHT);
        mTodayHeaderTextPaint.setTextSize(mTextSize);
        mTodayHeaderTextPaint.setTypeface(Typeface.createFromAsset(mContext.getAssets(),"fonts/Exo-Medium.otf"));
        mTodayHeaderTextPaint.setColor(mTodayHeaderTextColor);

        // Prepare today header text color paint.
        mTodayHeaderRightTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTodayHeaderRightTextPaint.setTextAlign(Paint.Align.CENTER);
        mTodayHeaderRightTextPaint.setTextSize(mTextSize-38);
        mTodayHeaderRightTextPaint.setTypeface(Typeface.createFromAsset(mContext.getAssets(),"fonts/Exo-Medium.otf"));
        mTodayHeaderRightTextPaint.setColor(mTodayHeaderTextColor);
...
//        canvas.drawRect(0, 0, mTimeTextWidth + mHeaderColumnPadding * 2, mHeaderTextHeight + mHeaderRowPadding * 2, mHeaderBackgroundPaint);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        mWidthPerDay = (getWidth() + (mTimeTextWidth + mHeaderColumnPadding * 2)) - mHeaderColumnWidth - mColumnGap * (mNumberOfVisibleDays - 1);
        ...
        if (mIsFirstDraw && mNumberOfVisibleDays >= 7) {
            if (mToday.get(Calendar.DAY_OF_WEEK) != mFirstDayOfWeek) {
                ...
                mCurrentOrigin.x += (mWidthPerDay + mColumnGap) * (difference - (mNumberOfVisibleDays+1));
            }
            mIsFirstDraw = false;
        }
...
//        }
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            // Draw the day labels.
//            String dayLabel = String.format("%d", getDayName(day), day.get(Calendar.MONTH) + 1, day.get(Calendar.DAY_OF_MONTH));

            String dayLabel = String.format("%d",  day.get(Calendar.DAY_OF_MONTH));
            canvas.drawText(dayLabel, startPixel + mWidthPerDay, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);


            Rect rect = new Rect();
            float widthCenter = (startPixel + mWidthPerDay) + mColumnGap/2;
            String l = getDayName(day).substring(0,3);
            for (int i = 0; i < l.length(); i++) {
                canvas.drawText(String.valueOf(l.charAt(i)), widthCenter, (mHeaderRightTextHeight + (i * mHeaderRightTextHeight))+mHeaderRowPadding, sameDay ? mTodayHeaderRightTextPaint : mHeaderRightTextPaint);
            }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            if (Math.abs(mScroller.getFinalX() - mScroller.getCurrX()) < (mWidthPerDay + mColumnGap) * (mNumberOfVisibleDays/2) && Math.abs(mScroller.getFinalX() - mScroller.getStartX()) != 0) {
                mScroller.forceFinished(true);
                float leftDays = Math.round(mCurrentOrigin.x / ((mWidthPerDay + mColumnGap) * mNumberOfVisibleDays));
                int nearestOrigin = (int) (mCurrentOrigin.x - leftDays * ((mWidthPerDay+mColumnGap) * mNumberOfVisibleDays));
                mStickyScroller.startScroll((int) mCurrentOrigin.x, 0, - nearestOrigin, 0);
                ViewCompat.postInvalidateOnAnimation(WeekView.this);
            }
            else {
